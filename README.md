...
ScriptForge 是一个 AI 辅助小说改编流水线工具。它把多章节小说拆解为章节摘要、人物表、地点表、冲突表、时间线、场景和剧本片段，并输出符合 YAML Schema 的结构化剧本初稿，方便作者继续编辑、校验和二次创作。
...
- `timeline`：按顺序记录剧情事件，可通过 `conflict_ids` 关联冲突。
- `conflicts`：冲突表，记录冲突类型、参与人物、风险、状态、来源章节和相关时间线事件。
- `scenes`：结构化剧本场景，包含来源章节、时间地点、人物、冲突、目的、节拍、动作、对白和改编策略。
...
- `conflict.source_chapters` 和 `conflict.related_timeline` 是否引用已存在章节和时间线事件。
